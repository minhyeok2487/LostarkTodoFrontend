...
class Interpolation$1 {
  ...
  static create(config, output, extra) {
    if (typeof config === 'function') return config;else if (interpolation && config.output && typeof config.output[0] === 'string') return interpolation(config);else if (Array.isArray(config)) return Interpolation$1.create({
      range: config,
      output,
...
class AnimatedInterpolation extends AnimatedArrayWithChildren {
  constructor(parents, _config, _arg) {
    ...
    this.updateConfig = (config, arg) => this.calc = Interpolation$1.create(config, arg);
